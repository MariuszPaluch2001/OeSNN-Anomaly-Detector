...
if __name__ == "__main__":
    ...
    path = "/mnt/c/workspace/oesnn/OeSNN-Anomaly-Detector/data/NAB/realTweets/realTweets_Twitter_volume_UPS.csv"
    data, labels = get_data_from_path(path, True)

    with open("results/decomp_nab_result.csv", "w", encoding='UTF8') as file:
        ...
        for path in get_all_files_paths("../data/NAB"):
            ...
            if df_stream['f1'] == 0.0:
                continue
            t = time.time()
            res = DecomposeOeSNNAD(stream, trials=100).predict(cemmndan_channels=-1,
            ...
            print(f"F1 {f1}, Recall {recall}, Precission {precission}")
            ...
            plt.savefig(f"plots-decomposition/NAB-{filename}.png")
            plt.close()

    with open("results/decomp_yahoo_result.csv", "w", encoding='UTF8') as file:
        writer = csv.writer(file)
        ...
        for path in get_all_files_paths("../data/Yahoo"):
            ...
            plt.xlabel("Timestamp")
            plt.ylabel("Input value (X)")
            plt.savefig(f"plots-decomposition/Yahoo-{filename}.png")
            plt.close()
